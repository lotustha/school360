...
export async function applyAllocations(
  ...
  const rows = await tx.studentFee.findMany({ where: { id: { in: ids }, schoolId } })
  ...
  for (const a of allocations) {
    ...
    if (row.status === "PLANNED")   throw new Error(`Row "${row.periodLabel}" is not yet BILLED — bill it first`)
...
export async function planFifoAllocations(
  ...
  const outstanding = await tx.studentFee.findMany({
    where: { schoolId, studentId, status: { in: ["BILLED", "PARTIAL"] } },
    orderBy: [
