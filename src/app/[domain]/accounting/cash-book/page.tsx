...
import { Printer, Coins, ArrowDownRight, ArrowUpRight, Wallet } from "lucide-react"
...
export default async function CashBookPage({
  ...
  let book
  try {
    book = await getCashBook(fyId!, sp.from, sp.to)
  } catch (e) {
    return (
      <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-amber-200/60 shadow-sm p-8 text-center max-w-2xl">
        ...
        <p className="text-xs text-muted-foreground mt-2">Add a CASH-subtype account in your Chart of Accounts.</p>
        <Link href="/accounting/accounts">
          <Button size="sm" className="mt-4 cursor-pointer">Open Chart of Accounts</Button>
        </Link>
      </div>
    )
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <ReportExportButton kind="cash-book" data={book} schoolName={school.name} />
          <Link href={`/accounting/cash-book/print?fy=${fyId}${sp.from ? `&from=${sp.from}` : ""}${sp.to ? `&to=${sp.to}` : ""}`} target="_blank">
      ...
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        {[
          ...
          return (
            <Link key={p.label} href={`/accounting/cash-book?${qs.toString()}`}>
      ...
      <BookView
        ...
        inLabel="Receipt"
        outLabel="Payment"
        emptyText="No cash activity in this period."
      />
    </div>
  )
}

// ─── KPI helpers (shared with bank-book via copy — small, simple) ───────────
