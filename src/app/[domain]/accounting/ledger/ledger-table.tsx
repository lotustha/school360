...
import type { LedgerRow } from "@/actions/accounting/reports"
...
export function LedgerTable({ rows, openingDebit, openingCredit, closingBalance }: Props) {
  const [search, setSearch] = useState("")
  const [filter, setFilter] = useState<"ALL" | "DR" | "CR">("ALL")

  const counts = useMemo(() => ({
    all: rows.length,
    dr:  rows.filter(r => parseFloat(r.debit) > 0).length,
    cr:  rows.filter(r => parseFloat(r.credit) > 0).length,
  }), [rows])

  const filteredRows = useMemo(() => {
    const q = search.trim().toLowerCase()
    return rows.filter(r => {
      if (filter === "DR" && !(parseFloat(r.debit) > 0)) return false
      if (filter === "CR" && !(parseFloat(r.credit) > 0)) return false
      if (!q) return true
      return (
  ...
  const subTotals = useMemo(() => {
    return filteredRows.reduce((a, r) => ({
      dr: a.dr + (parseFloat(r.debit) || 0),
      cr: a.cr + (parseFloat(r.credit) || 0),
    }), { dr: 0, cr: 0 })
  }, [filteredRows])
    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/40 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-3 border-b border-white/60 flex items-center gap-2 flex-wrap">
        <div className="inline-flex bg-slate-100/60 rounded-lg p-0.5">
          {([
            ...
            { k: "DR",  label: "DEBIT",   count: counts.dr },
            { k: "CR",  label: "CREDIT",  count: counts.cr },
          ] as const).map(o => (
            <button
              key={o.k}
              type="button"
      ...
      {rows.length === 0 ? (
        <div className="p-16 text-center">
          ...
          <p className="text-sm text-slate-600">No activity for this account in this period.</p>
          <p className="text-xs text-slate-400 mt-1">Try a different date range or fiscal year.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50/80 text-[10px] uppercase tracking-widest text-slate-500 font-black sticky top-0 z-10 backdrop-blur-xl">
              <tr>
                ...
                <th className="px-4 py-3 text-right w-28" title="Debit — increases assets/expenses, decreases liabilities/income">Debit</th>
                <th className="px-4 py-3 text-right w-28" title="Credit — opposite of debit">Credit</th>
            ...
            <tbody className="divide-y divide-slate-100/60">
              ...
              <tr className="bg-slate-50/40 font-semibold border-b border-slate-200">
                <td colSpan={2} className="px-4 py-2 text-right text-[10px] uppercase tracking-widest text-slate-500">Opening balance</td>
                <td className="px-4 py-2 text-right font-mono tabular-nums">{parseFloat(openingDebit) > 0 ? openingDebit : <span className="text-slate-300">—</span>}</td>
                <td className="px-4 py-2 text-right font-mono tabular-nums">{parseFloat(openingCredit) > 0 ? openingCredit : <span className="text-slate-300">—</span>}</td>
                <td className="px-4 py-2 text-right font-mono tabular-nums">
                  {parseFloat(openingDebit) > 0
                    ? openingDebit
                    : parseFloat(openingCredit) > 0
                      ? `(${openingCredit})`
                      : "0.00"}
                </td>
              </tr>
              ...
              {grouped.flatMap(([date, dateRows]) => [
                ...
                ...dateRows.map((r, i) => {
                  ...
                  const running = parseFloat(r.running)
                  return (
                    <tr key={`${date}-${i}-${r.voucherId}`} className="hover:bg-primary/4 transition-colors">
                      ...
                      <td className={cn(
                        "px-4 py-2 text-right font-mono tabular-nums font-bold",
                        running >= 0 ? "text-slate-900" : "text-rose-700",
                      )}>
                        {running >= 0 ? r.running : `(${Math.abs(running).toFixed(2)})`}
                      </td>
                    </tr>
                  )
                }),
            ...
            <tfoot className="bg-slate-50/80 font-bold sticky bottom-0 backdrop-blur-xl">
              {showSubtotals && (
                <tr>
                  ...
                  <td className="px-4 py-2 text-right font-mono tabular-nums text-emerald-700">{subTotals.dr.toFixed(2)}</td>
                  <td className="px-4 py-2 text-right font-mono tabular-nums text-rose-700">{subTotals.cr.toFixed(2)}</td>
                  <td></td>
                </tr>
              )}
              <tr className="border-t-2 border-slate-200">
                <td colSpan={2} className="px-4 py-2.5 text-right text-xs uppercase tracking-widest text-slate-500">Period totals</td>
                <td className="px-4 py-2.5 text-right font-mono tabular-nums text-emerald-700">{periodTotals.dr.toFixed(2)}</td>
                <td className="px-4 py-2.5 text-right font-mono tabular-nums text-rose-700">{periodTotals.cr.toFixed(2)}</td>
                <td className="px-4 py-2.5 text-right font-mono tabular-nums font-black">
                  {(() => {
                    const c = parseFloat(closingBalance)
                    if (c >= 0) return `${closingBalance} Dr`
                    return `${Math.abs(c).toFixed(2)} Cr`
                  })()}
                </td>
              </tr>
            </tfoot>
      ...
      <div className="px-5 py-2.5 border-t border-slate-100/80 bg-slate-50/40 text-[11px] text-slate-500 flex items-center justify-between gap-2 flex-wrap">
        <span>
          Numbers in parentheses indicate a <strong className="text-rose-700">credit (Cr) balance</strong>.
          Numbers without indicate a <strong className="text-emerald-700">debit (Dr) balance</strong>.
        </span>
      </div>
    </div>
  )
