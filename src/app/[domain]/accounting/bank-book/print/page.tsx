...
import { getBankBook } from "@/actions/accounting/reports"
...
export default async function Print({
  ...
  if (!fyId) notFound()
  const book = await getBankBook(fyId, sp.from, sp.to, sp.account)
      ...
      <table className="w-full text-sm border border-slate-400">
        <thead>
          <tr className="bg-slate-100 border-b border-slate-400">
            ...
            <th className="px-2 py-1.5 text-right text-[10px] uppercase tracking-wider font-bold w-24">Deposit (Dr)</th>
            <th className="px-2 py-1.5 text-right text-[10px] uppercase tracking-wider font-bold w-24">Withdrawal (Cr)</th>
